...
export class graph implements OnChanges, AfterViewInit {
  ...
  private buildChart() {
    ...
    let line = D3.line()
      .curve(
        this.curveArray.find(item => item.curveTitle === this.selectedCurve)
    ...
    D3.tsv("/src/app/Graph/testdata.tsv", this.type, function(error, data) {
      if (error) {
        throw error;
      }
